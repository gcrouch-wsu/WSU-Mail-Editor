...
def apply_config_defaults(model_data, template_type):
    ...
    if template_type == 'ff':
        ...
        model_data['masthead']['tagline'] = model_data['masthead'].get('tagline', FRIDAY_FOCUS['tagline'])
        model_data['masthead']['preheader'] = model_data['masthead'].get('preheader', FRIDAY_FOCUS['preheader'])
    elif template_type == 'briefing':
        model_data['masthead']['title'] = model_data['masthead'].get('title', BRIEFING['title'])
        model_data['masthead']['tagline'] = model_data['masthead'].get('tagline', BRIEFING['tagline'])
        model_data['masthead']['preheader'] = model_data['masthead'].get('preheader', BRIEFING['preheader'])
    
    # Apply branding defaults
    model_data['masthead']['banner_url'] = model_data['masthead'].get('banner_url', DEFAULT_BANNER_URL)
    model_data['masthead']['banner_alt'] = model_data['masthead'].get('banner_alt', DEFAULT_BANNER_ALT)
                ...
                "cards": [
                    {
                        ...
                        "icon_size": 80,
                        "links": [{"label": "Read more", "url": RESOURCE_LINKS['health_counseling']['url']}],
                        "spacing_bottom": 20
                    },
                    {
                        "type": "resource",
                        ...
                        "icon_size": 80,
                        "links": [{"label": "Read more", "url": RESOURCE_LINKS['basic_needs']['url']}],
                        "spacing_bottom": 20
                    },
                    {
                        "type": "resource",
                        ...
                        "icon_size": 80,
                        "links": [{"label": "Read more", "url": RESOURCE_LINKS['career_services']['url']}],
                        "spacing_bottom": 20
                    }
                ]
            },
                ...
                "cards": [
                    {
                        ...
                        "links": [{"label": "Please use this form", "url": FRIDAY_FOCUS['submit_form_url']}]
                    }
                ]
            }
        ],
                ...
                "cards": [
                    {
                        ...
                        "body_html": "<p>Do you have an update or announcement to share? We encourage submissions from all graduate programs.</p>",
                        "links": [{"label": "Submit your post", "url": BRIEFING['submit_form_url']}],
                        "spacing_bottom": 20
                    }
                ]
            },
...
def api_export():
    ...
    try:
        ...
        app.logger.info(f"Generating HTML for template: {newsletter_data.get('template', 'unknown')}")
        ...
        if minify:
            ...
            app.logger.info(f"Minified HTML size: {minified_size} bytes (saved {original_size - minified_size} bytes)")
        ...
        if not strip_json:
            ...
            app.logger.info(f"Embedded JSON data: {len(json_str)} bytes → {len(json_b64)} bytes (Base64)")
    ...
    try:
        ...
        if 'footer' in newsletter_data and 'social' in newsletter_data['footer']:
            ...
            if isinstance(social, dict) and not isinstance(social, list):
                ...
                app.logger.info(f"✅ Migrated {len(migrated_social)} social links from V6 to V7 format")
        ...
        if 'sections' in newsletter_data:
            for section in newsletter_data['sections']:
                if 'layout' in section and 'title_align' not in section['layout']:
                    ...
                    app.logger.info(f"✅ Added default title_align to section: {section.get('title', 'untitled')}")
