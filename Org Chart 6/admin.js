...
let HELPER = { text:"Use the circle icons to expand or collapse the tree.", size:14 };
let CURRENT_LAYOUT = "centered";
let SELF_CONTAINED_EXPORT = false;
...
function loadRuntime() {
  ...
  link.id = "runtime-css";
  link.rel = "stylesheet";
  link.href = "/runtime.css?type=" + CURRENT_LAYOUT + "&cb=" + Date.now();
  ...
  setTimeout(() => {
    ...
    s.id = "runtime";
    s.src = "/runtime.js?type=" + CURRENT_LAYOUT + "&cb=" + Date.now();
    s.onload = () => {
...
async function buildSelfContainedExport(baseHTML, closeScript) {
  ...
  try {
    const cssResp = await fetch("/runtime.css?type=" + CURRENT_LAYOUT);
    if (!cssResp.ok) {
  ...
  try {
    const jsResp = await fetch("/runtime.js?type=" + CURRENT_LAYOUT);
    if (!jsResp.ok) {
...
document.getElementById("btnUploadImport").addEventListener("click", async ()=>{
...
document.getElementById("btnLoad").addEventListener("click", async ()=>{
  const t = await (await fetch("/sample?type=" + CURRENT_LAYOUT)).text(); 
...
document.getElementById("btnDelete").addEventListener("click", delNode);
document.getElementById("btnSave").addEventListener("click", saveInspector);
document.getElementById("btnRevert").addEventListener("click", revertInspector);

document.getElementById("selL1").addEventListener("change", ()=>{
  const v=document.getElementById("selL1").value; 
  if(v){ SELECTED_ID=v; hydrateInspector(); renderNodeDropdowns(); updateL1Buttons(); }
});
document.getElementById("btnL1Up").addEventListener("click", ()=>l1Move(-1));
document.getElementById("btnL1Down").addEventListener("click", ()=>l1Move(1));
document.getElementById("btnL1Flip").addEventListener("click", l1Flip);
...
document.getElementById("helperText").addEventListener("input", ()=>{ 
  HELPER.text = document.getElementById("helperText").value; 
  applyHelperToPreview(); 
});
document.getElementById("helperSize").addEventListener("input", ()=>{ 
  HELPER.size = parseInt(document.getElementById("helperSize").value||"14",10); 
  applyHelperToPreview(); 
});

document.getElementById("nodeSelect").addEventListener("change", ()=>{
  SELECTED_ID = document.getElementById("nodeSelect").value || null;
  hydrateInspector(); renderNodeDropdowns(); updateL1Buttons();
});

document.getElementById("selfContainedExport").addEventListener("change", (e) => {
...
document.getElementById("helperText").value = HELPER.text;
document.getElementById("helperSize").value = String(HELPER.size);
